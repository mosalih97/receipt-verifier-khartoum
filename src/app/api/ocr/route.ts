...
// في الإنتاج: استخدم Redis أو قاعدة بيانات خارجية
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data } = await Tesseract.recognize(image, 'ara+eng', {
      logger: (m) => console.log('[Tesseract]', m), // للتصحيح في Vercel logs
    });
    ...
    // اختيار أفضل حساب (الأقرب للنص "إلى حساب")
    ...
    // === التحقق من الاسم بدقة (كل كلمة موجودة، بدون أسماء طويلة جدًا) ===
    ...
    const matched =
      accountMatch && nameMatchResult && timeValid && noDuplicate && hasTransactionId;
